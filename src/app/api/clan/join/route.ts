...
export async function POST(request: NextRequest) {
  try {
    ...
    const { username, clan: userClan }: any = jwt.verify(token, JWT_SECRET)
    ...
    if (userClan?.joined) return NextResponse.json({ error: "Already joined another clan" }, { status: 400, statusText: "Already joined another clan" })
