...
export default function WithdrawPage() {
  ...
  const [dpVsBt, setDpVsBt] = useState<{ deposit: number, bet: number }>()
  ...
  useEffect(() => {
    axios.get("/api/withdraw/condition", { headers: { token: localStorage.getItem("jwt") } })
      .then(({ data: depositVsBet }) => {
        setDpVsBt(depositVsBet)
      })
  }, [])
  useEffect(() => {
    if (!network) return
        ...
        <div className="flex justify-center">
          ...
          <div className="flex gap-2">
            ...
            <p className="text-sm">Request through gamecurrency</p>
              ...
              {dpVsBt && dpVsBt.bet < dpVsBt.deposit &&
              ...
              {sendingRequest ?
                ...
                <button onClick={handleWithdraw} className="w-full py-2 bg-[#01A3DB] hover:bg-[#45bce4] cursor-pointer disabled:cursor-not-allowed disabled:bg-[#01A3DB44] border border-[#364153] rounded-[10px] text-sm font-semibold" disabled={sendingRequest || (dpVsBt && dpVsBt.bet < dpVsBt.deposit)}>
                  Request Withdrawal
                </button>
              }
