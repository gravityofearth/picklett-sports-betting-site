...
export const sendVerifyEmail = ({ link, username, email }: { link: string, username: string, email: string }) => {
    axios.post("https://send.api.mailtrap.io/api/send", {
        ...
        text: `Verify your email address using this link: ${link}`,
        html: `<html><body><div style="font-family: system-ui, sans-serif, Arial; font-size: 14px; color: #333; padding: 20px 14px; background-color: #f5f5f5;"><div style="max-width: 600px; margin: auto; background-color: #fff"><div style="text-align: center; background-color: #333; padding: 14px"><a style="text-decoration: none; outline: none" href="https://www.picklett.com" target="_blank"><span style="color:#fff; font-size:20px; vertical-align:middle;">Picklett</span></a></div><div style="padding: 14px"><h1 style="font-size: 22px; margin-bottom: 26px">Confirm Your Email Address</h1><p>Welcome to Picklett!</p><p>To finish setting up your account, please confirm your email address by clicking the button below. This will allow you to get started with our platform.</p><a href="${link}" target="_blank" style="cursor:pointer; text-decoration: none; opacity: 1;background-color: #1187C1;opacity: 1;color: white;font-size: 16px;font-weight: 700;font-style: normal;letter-spacing: 0.07px;text-align: center;padding: 10px 20px;cursor: pointer;border: 0px;"> Confirm Email Address </a><p>If you didn't request this email verification, please ignore this email. Your account remains secure.</p><p>Best regards,<br />Picklett Team</p></div></div><div style="max-width: 600px; margin: auto; color: #999"><p> The email was sent to ${email}<br /> You received this email because you are registered with Picklett.com </p><p> If you're having trouble clicking button, copy and paste the URL below into your web browser: ${link}</p></div></div></body></html>`
    }, {
        headers: {
